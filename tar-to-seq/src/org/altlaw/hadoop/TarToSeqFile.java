    ...
    public void execute() throws Exception {
        ...
        try {
            ...
            while ((entry = input.getNextEntry()) != null) {
                ...
                //String data = new String(TarToSeqFile.getBytes(input, entry.getSize()), "UTF-8");
                
                Text key = new Text(filename + "__key");
                String prepend = "__data";
                byte[] prependBytes = prepend.getBytes();
                byte[] marked = new byte[data.length + prependBytes.length];
                System.arraycopy(prependBytes, 0, marked, 0, prependBytes.length);
                System.arraycopy(data, 0, marked, prependBytes.length, data.length);
                BytesWritable value = new BytesWritable(marked);
